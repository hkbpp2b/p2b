...
const HeroCard = ({ onSelect }: HeroCardProps) => {
    ...
    const handlePhotoClick = (slide: any) => {
        if (window.innerWidth >= 1024 && onSelect) {
            onSelect(slide);
        } else {
            setSelectedPhoto(slide);
        }
    };
                ...
                if (rawRows.length > 1) {
                    ...
                    rawRows.forEach((cols, index) => {
                        ...
                        const isInvalid = (val: string) => {
                            ...
                            return !val || val === "-" || val === "" || cleanVal === "judul:" || cleanVal === "judul";
                        };
    ...
    return (
        <>
                ...
                <div className="mt-4 px-2 space-y-1 pb-1 text-slate-900">
                    ...
                    <div className="mt-3 pt-3 border-t border-slate-100/60">
                        <div className="flex gap-2 px-1">
                            <a href="https://www.instagram.com/hkbp.perum2bks/" target="_blank" rel="noopener noreferrer" className="flex-1 py-3 bg-rose-50 rounded-2xl flex justify-center items-center active:bg-rose-100 active:scale-95 transition-all">
                                <Instagram size={18} className="text-pink-600" />
                            </a>
                            <a href="https://www.youtube.com/@HKBPPerumnas2Bekasi" target="_blank" rel="noopener noreferrer" className="flex-1 py-3 bg-red-50 rounded-2xl flex justify-center items-center active:bg-red-100 active:scale-95 transition-all">
                                <Youtube size={18} className="text-red-600" />
                            </a>
                            <a href="https://www.facebook.com/hkbp.perum2bks" target="_blank" rel="noopener noreferrer" className="flex-1 py-3 bg-blue-50 rounded-2xl flex justify-center items-center active:bg-blue-100 active:scale-95 transition-all">
                                <Facebook size={18} className="text-blue-600" />
                            </a>
                            <a href="https://www.tiktok.com/@hkbp.perum2bks" target="_blank" rel="noopener noreferrer" className="flex-1 py-3 bg-slate-600 rounded-2xl flex justify-center items-center active:bg-slate-800 active:scale-95 transition-all">
            ...
            {selectedPhoto && (
                <div className="fixed inset-0 z-[999] bg-white flex flex-col animate-in fade-in duration-300 lg:hidden">
                    <div className="w-full max-w-7xl mx-auto flex flex-col h-full overflow-hidden">
                        <div className={`flex items-center px-6 h-[75px] bg-white sticky top-0 z-50 transition-all duration-300 ${isScrolled ? 'border-b border-slate-100 shadow-sm' : 'border-transparent'}`}>
                            <button onClick={() => setSelectedPhoto(null)} className="p-2 -ml-2">
                                <ArrowLeft size={24} />
                            </button>
                        ...
                        <div ref={scrollRef} onScroll={handleScroll} className="flex-1 overflow-y-auto no-scrollbar bg-white">
                            <div className="flex flex-col items-start min-h-full">
                                <div className="w-full h-[350px] overflow-hidden bg-slate-50">
                                    <img src={selectedPhoto.url} className="w-full h-full object-cover" alt="Content" />
                                </div>
                                <div className="w-full p-8 flex flex-col bg-white">
                                    <div className="mb-8">
                                        <p className="text-[12px] font-black text-blue-600 uppercase tracking-[0.3em] mb-2">{selectedPhoto.tanggal}</p>
                                        <h2 className="text-2xl font-black text-slate-900 tracking-tighter uppercase leading-[1.1]">{selectedPhoto.judul}</h2>
                                    </div>
                                    <div className="pb-32">
                                        <p className="text-[16px] font-bold text-slate-900 leading-[1.8] text-left whitespace-pre-line">{selectedPhoto.deskripsi}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
